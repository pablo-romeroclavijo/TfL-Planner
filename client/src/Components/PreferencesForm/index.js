...
export default function PreferencesForm() {
	...
	async function handleFormSubmit() {
		try {
			...
			if (response.status === 200) {
				Alert.alert("Submit Successful", "Your Preferences have been updated", [{ text: "OK", onPress: () => closeModal() }]);
			}
		} catch (error) {
			...
			<GestureRecognizer style={{ flex: 1 }} onSwipeDown={() => closeModal()}>
				<Modal animationType="slide" transparent={false} visible={modalVisible} onRequestClose={closeModal}>
					<View style={styles.modalContainer}>
						...
						<AppTextInput placeholder="Postcode" icon="post" onChangeText={(text) => setPostcodeInput(text)} value={postcodeInput} />
